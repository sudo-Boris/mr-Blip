...
from lavis.tasks.moment_retrieval import filter_post_process_gpt4o, remove_special_chars
from lavis.processors.gpt4o_processors import Gpt4oVideoEvalProcessor
...
def post_process_gpt4o(pred):
    ...
    # pred = pred.split("</s>")[0]
    # import pdb; pdb.set_trace() # does it have </s>? does it have ANSWER:?
    # print("pred:", pred)
    pred = pred.split("ANSWER")[-1].split("Answer")[-1].split(':')[-1]
    ...
    # # if not, return "[[-1, -1]]"
    # # we are doing single batch_size
    # # if not re.match(r"\[\[.*\]\]", pred):
    # if not re.match(r"\[.*\]", pred):
    #     import pdb; pdb.set_trace()
    #     return "[[-1, -1]]"
    print("pred:", pred)
    pred = filter_post_process_gpt4o(pred)
    ...
    windows = [windows[-1]] # take only the last window
...
def format_wandb_log_images_and_predictions(
    ...
    if (
        ...
        import pdb; pdb.set_trace()
        processed_pred = convert_to_absolute_time([processed_pred], [duration], input_time_format)
...
def convert_to_absolute_time(prediction, duration, input_time_format, significant_digits=2):
    ...
    for pred, dur in zip(prediction, duration):
        print("before: pred:", pred, "dur:", dur)
        if input_time_format == "relative_integers":
            absolute_prediction.append(
                [
                    (
                        [
                            round((float(start) / 100) * dur, significant_digits),
                            round((float(end) / 100) * dur, significant_digits),
                        ]
            ...
            absolute_prediction.append(
                [
                    (
                        [round(float(start) * dur, significant_digits), round(float(end) * dur, significant_digits)]
                        if start != -1 and end != -1
                        else [-1, -1]
                    )
                    for start, end in pred
        ...
        print("after: absolute_prediction:", absolute_prediction)
...
def get_timestamps_as_relative_integers(
    timestamps, durations, annoying_numbers_replacement_dict=None, significant_digits=2
):
    ...
    for t, d in zip(timestamps, durations):
        ...
        _video_prompt = ">".join(
            str(int(round((timestamp.item() / duration), significant_digits) * 100)) for timestamp in t
        )
        # add the video duration
        ...
        new_timestamps.append(
            torch.tensor(
                [int(round((timestamp.item() / duration), significant_digits) * 100) for timestamp in t]
            )
        )
...
def get_frames(video_path, start_time, end_time, n_frames=4, return_torch=True):
    """
    ...
    if n_frames > 1:
        ...
        frames = np.asarray([frames[0].to_ndarray(format="rgb24")])
    if return_torch:
...
def get_frames_gpt4o(video_path, start_time, end_time, n_frames=4):
    frames = get_frames(video_path, start_time, end_time, n_frames, return_torch=False)
    frames = Gpt4oVideoEvalProcessor.clip_to_base64frames(frames)

    return frames


def post_process_gpt4o_qa(m, options="A,B,C,D"):
    m = m.split("ANSWER")[-1].split("Answer")[-1].split(':')[-1].strip()
    m = remove_special_chars(m)
    options_list = options.split(",")
    for c in options_list:
        if c == m:
            return c
    import pdb; pdb.set_trace() # it should have matched already