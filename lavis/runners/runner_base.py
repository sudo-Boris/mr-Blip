...
class RunnerBase:
    ...
    def eval_epoch(self, split_name, cur_epoch, skip_reload=False):
        ...
        """
        results_path = self.config.config['run'].get("results_path")
        if self.config.config['run'].get("from_file", False):
            if not results_path:
                raise ValueError("results_path must be specified when evaluate_only is True")
            ...
            if model: # TODO: somehow model is None in gpt4o. not sure where self.model is loaded
                model.eval()
            ...
            results = self.task.evaluation(
                model, data_loader, cuda_enabled=torch.cuda.is_available(), 
                save_every_sample=self.config.config['run'].get("save_every_sample", False)
            )
        ...
        if results is not None:
            return self.task.after_evaluation(
                ...
                epoch=cur_epoch,
                from_file=self.config.config['run'].get("from_file", False),
                results_path=results_path,
                allow_multiple_pred_windows=self.config.config['model'].get("use_multiple_moments", False),
                **self.config.config['run'].get('eval', {})
            )
