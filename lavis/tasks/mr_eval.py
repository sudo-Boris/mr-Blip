...
def ensure_windows(windows_candidate, index=-1, multiple_pred_windows=False):
    if isinstance(windows_candidate, (list, np.ndarray)) and len(windows_candidate) == 2 \
        and isinstance(windows_candidate[0], (int, float, np.integer, np.floating)):
        # return [windows_candidate]
        return windows_candidate
    elif isinstance(windows_candidate, (list, np.ndarray)) \
        and isinstance(windows_candidate[0], (list, np.ndarray)) \
        and isinstance(windows_candidate[0][0], (int, float, np.integer, np.floating)):
        # return windows_candidate
        # return random.choice(windows_candidate)
        return windows_candidate[index] if not multiple_pred_windows else windows_candidate
    elif isinstance(windows_candidate, (list, np.ndarray)) \
        and isinstance(windows_candidate[0], (list, np.ndarray)) \
        and isinstance(windows_candidate[0][0], (list, np.ndarray)) \
        and isinstance(windows_candidate[0][0][0], (int, float, np.integer, np.floating)):
        assert len(windows_candidate) == 1, f"Expected batch_size 1"
        return windows_candidate[index] if multiple_pred_windows else [windows_candidate[index][index]]
    else:
        # return [windows_candidate]
        import pdb; pdb.set_trace()
        raise ValueError(f"Invalid windows candidate: {windows_candidate}")

def compute_mr_r1(submission, ground_truth, iou_thds=np.linspace(0.5, 0.95, 10), multiple_pred_windows=False):
    """If a predicted segment has IoU >= iou_thd with one of the 1st GT segment, we define it positive"""
    import os
    deduplicate_sample_keys = bool(int(os.environ.get("DEDUPLICATE_SAMPLE_KEYS", 1)))

    iou_thds = [float(f"{e:.2f}") for e in iou_thds]
    if deduplicate_sample_keys:
        gt_qid2window = {}
        pred_windows_list, gt_windows = [], []
        for d in ground_truth:
            cur_gt_windows = d["relevant_windows"]
            cur_qid = d["qid"]
            cur_max_iou_idx = 0
            if len(cur_gt_windows) > 0:  # select the GT window that has the highest IoU
                i_pred_window = np.array(d["pred_relevant_windows"][0][:2])
                if i_pred_window.shape == (2,):
                    i_pred_window = i_pred_window[None, :]
                elif i_pred_window.shape == (1, 2):
                    pass
                else:
                    raise ValueError(f"Invalid pred_relevant_windows shape: {i_pred_window.shape}")
                cur_ious = compute_temporal_iou_batch_cross(
                     i_pred_window, np.array(cur_gt_windows)
                )[0]
                cur_max_iou_idx = np.argmax(cur_ious)
            
            gt_qid2window[cur_qid] = cur_gt_windows[cur_max_iou_idx]
            pred_windows_list.append(
                ensure_windows(i_pred_window, 
                               multiple_pred_windows=multiple_pred_windows))
            gt_windows.append(cur_gt_windows[cur_max_iou_idx])
        gt_windows = np.array(gt_windows).astype(float)
        ...
        pred_windows_list = [ensure_windows(pred_qid2window[k]) for k in qids]
    ...
    for thd in iou_thds:
        print(f"Computing R1@IoU using {len(pred_gt_iou)} samples")
...
def eval_moment_retrieval(submission, ground_truth, verbose=True):
    ...
    for name in range_names:
        ...
        iou_thd2average_precision = compute_mr_ap(
            ...
            _submission, _ground_truth, num_workers=1, chunksize=1
        )
