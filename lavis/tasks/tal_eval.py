# For Mr. BLIP: Code adapted from https://github.com/happyharrycn/actionformer_release/blob/main/libs/utils/metrics.py
...
class ANETdetection(object):
    ...
    def __init__(
        ...
        self.mAP_over_videos = mAP_over_videos
        if dataset_name is not None:
            self.dataset_name = dataset_name
        else:
            self.dataset_name = os.path.basename(targets).replace(".json", "")

        # Import ground truth and predictions
        self.split = split

        if isinstance(targets, str) and os.path.isfile(targets):
            self.ground_truth = load_gt_seg_from_json(
                targets, split=self.split, label=label, label_offset=label_offset
            )
        elif isinstance(targets, Dict):
            # move to pd dataframe
            self.ground_truth = pd.DataFrame(
                {
                    "video-id": targets["video-id"],
                    "t-start": targets["t-start"],
                    "t-end": targets["t-end"],
                    "label": targets["label"],
                }
            )
    ...
    def wrapper_compute_average_precision_over_videos(self, preds):
        """Computes average precision for each video in the subset."""

        # Adaptation to query faster
        ground_truth_by_video_id = self.ground_truth.groupby("video-id")
        prediction_by_video_id = preds.groupby("video-id")

        ap = np.zeros((len(self.tiou_thresholds), len(prediction_by_video_id)))

        results = Parallel(n_jobs=self.num_workers)(
            delayed(compute_average_precision_detection)(
                ground_truth=ground_truth_by_video_id.get_group(video_id).reset_index(
                    drop=True
                ),
                prediction=video_preds.reset_index(drop=True),
                tiou_thresholds=self.tiou_thresholds,
            )
            for video_id, video_preds in prediction_by_video_id
        )

        for i in range(len(prediction_by_video_id)):
            ap[:, i] = results[i]
    ...
    def wrapper_compute_topkx_recall(self, preds):
        ...
        recall = np.zeros(
            (len(self.tiou_thresholds), len(self.top_k), len(self.activity_index))
        )
    ...
    def evaluate(self, preds, verbose=True):
        ...
        # compute mAP
        if self.mAP_over_videos:
            self.ap = self.wrapper_compute_average_precision_over_videos(preds)
        else:
        ...
        # return the results
        return mAP, average_mAP, mRecall, self.ap, self.activity_index
