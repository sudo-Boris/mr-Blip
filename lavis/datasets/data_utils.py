...
def load_video(
    ...
    sampling="uniform",
    clip_proposal=None,
    type="torch", # [torch | numpy]
):  
    assert type in ["torch", "numpy"]
    if type == "torch":
        decord.bridge.set_bridge("torch")
    ...
    assert n_frms or fps, f"Must specify either n_frms or fps, but got n_frms={n_frms} and fps={fps}"
    if fps:
        n_frames = int(vlen * fps/original_fps)
    if n_frms:
        n_frames = min(n_frms, n_frames)
    indices = get_indices(vlen, n_frames, sampling, clip_proposal, original_fps)
    # get_batch -> T, H, W, C
    batch = vr.get_batch(indices)
    if type == "numpy":
        frms = batch.asnumpy().transpose(3, 0, 1, 2).astype(np.float32)  # (C, T, H, W)
    elif type == "torch":
        frms = batch.permute(3, 0, 1, 2).float()  # (C, T, H, W)

    return frms, indices, original_fps

def get_indices(vlen, n_frms, sampling, clip_proposal, fps):
...
def load_video_demo(
    ...
    if fps is not None:
        raise NotImplementedError("fps is not supported for load_video_demo")
    vr = VideoReader(uri=video_path, height=height, width=width)
    vlen = len(vr)
...
def slice_annotation_data(annotation_data, start_idx=None, end_idx=None):
    """
    Slice annotation data based on start and end indices.
    
    Args:
        annotation_data: List or dict of annotations
        start_idx: Starting index (inclusive)
        end_idx: Ending index (exclusive)
    
    Returns:
        Sliced annotation data
    """
    if start_idx is None and end_idx is None:
        return annotation_data
        
    if isinstance(annotation_data, list):
        return annotation_data[start_idx:end_idx]
    elif isinstance(annotation_data, dict):
        # For dictionary-type annotations, slice the values for each key
        sliced_data = {}
        ...
        raise ValueError(f"Unsupported annotation data type: {type(annotation_data)}")
